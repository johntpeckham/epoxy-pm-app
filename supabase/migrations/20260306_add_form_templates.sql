-- Form templates table for Form Management
-- Stores customizable field definitions for each app form
...
-- All authenticated users can read form templates
create policy "Authenticated users can read form_templates"
  on form_templates for select
  to authenticated
  using (true);

-- Admin and Office Manager can insert
create policy "Admin and Office Manager can insert form_templates"
  on form_templates for insert
  to authenticated
  with check (
    exists (
      select 1 from profiles
      where id = auth.uid() and role in ('admin', 'office_manager')
    )
  );

-- Admin and Office Manager can update
create policy "Admin and Office Manager can update form_templates"
...
create policy "Admin and Office Manager can delete form_templates"
...
-- Seed form templates with current field structures
insert into form_templates (form_key, form_name, fields) values
(
  'daily_report',
  'Daily Report',
  '[
