...
import { RulerIcon, UploadIcon, XIcon, Loader2Icon } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import ConfirmDialog from '@/components/ui/ConfirmDialog'
import PdfThumbnail from '@/components/documents/PdfThumbnail'
import AutoSaveIndicator from '@/components/ui/AutoSaveIndicator'
import type { ProjectTakeoffMeasurementPdf } from './types'

interface ProjectMeasurementsCardProps {
  projectId: string
  initialMeasurements: string | null
  userId: string
...
const BUCKET = 'project-takeoff-measurements'
...
export default function ProjectMeasurementsCard({
  ...
  const [pdfs, setPdfs] = useState<ProjectTakeoffMeasurementPdf[]>([])
  const [loadingPdfs, setLoadingPdfs] = useState(true)
  const [pending, setPending] = useState<PendingUpload[]>([])
  const [confirmDelete, setConfirmDelete] = useState<ProjectTakeoffMeasurementPdf | null>(null)
  const [deleting, setDeleting] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)

  useEffect(() => {
    setMeasurements(initialMeasurements ?? '')
  }, [projectId, initialMeasurements])

  const fetchPdfs = useCallback(async () => {
    setLoadingPdfs(true)
    const supabase = createClient()
    const { data, error } = await supabase
      .from('project_takeoff_measurement_pdfs')
      .select('*')
    ...
    if (error) {
      ...
      setPdfs(data as ProjectTakeoffMeasurementPdf[])
    }
  ...
  function handleTextChange(value: string) {
    ...
    saveTimerRef.current = setTimeout(async () => {
      ...
      if (error) {
        ...
        onMeasurementsChange(value || null)
        setSaveState('saved')
      ...
      await Promise.all(
        files.map(async (file, idx) => {
          ...
          try {
            ...
            if (inserted) {
              setPdfs((prev) => [...prev, inserted as ProjectTakeoffMeasurementPdf])
            }
          } catch (err) {
  ...
  async function handleDeletePdf(pdf: ProjectTakeoffMeasurementPdf) {
    setDeleting(true)
    const supabase = createClient()
    try {
            ...
            <div className="flex flex-wrap gap-3">
              {pdfs.map((pdf) => (
                <div key={pdf.id} className="flex flex-col gap-1.5 w-[120px]">
                  ...
                  <p className="text-xs text-gray-600 truncate" title={pdf.file_name}>
                    {pdf.file_name}
                  </p>
                </div>
              ))}
