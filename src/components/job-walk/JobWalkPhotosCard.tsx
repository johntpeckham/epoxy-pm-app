...
interface JobWalkPhotosCardProps {
  walkId: string
  userId: string
}

const BUCKET = 'job-walk-photos'

export default function JobWalkPhotosCard({ walkId, userId }: JobWalkPhotosCardProps) {
  const [photos, setPhotos] = useState<JobWalkPhoto[]>([])
  const [loading, setLoading] = useState(true)
  const [pending, setPending] = useState<PendingUpload[]>([])
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null)
  const [confirmDelete, setConfirmDelete] = useState<JobWalkPhoto | null>(null)
  ...
  const fetchPhotos = useCallback(async () => {
    ...
    if (error) {
      console.error('[JobWalkPhotos] Fetch failed:', error)
    } else if (data) {
      setPhotos(data as JobWalkPhoto[])
    }
    setLoading(false)
  }, [walkId])
  ...
  const handleFiles = useCallback(
    async (fileList: FileList | null) => {
      ...
      // Create pending placeholders with blob previews
      ...
      await Promise.all(
        files.map(async (file, idx) => {
          ...
          try {
            ...
            const path = `${walkId}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`
            ...
            if (inserted) {
              setPhotos((prev) => [...prev, inserted as JobWalkPhoto])
            }
          } catch (err) {
            console.error('[JobWalkPhotos] Upload failed:', err)
          } finally {
  ...
  async function handleDelete(photo: JobWalkPhoto) {
    setDeleting(true)
    const supabase = createClient()
    try {
      // Remove from storage first (ignore missing-file errors)
      ...
      console.error('[JobWalkPhotos] Delete failed:', err)
    } finally {
      setDeleting(false)
    }
  }

  function startEditCaption(photo: JobWalkPhoto) {
    setEditingId(photo.id)
    setEditingValue(photo.caption ?? '')
  }

  async function commitCaption(photo: JobWalkPhoto) {
    ...
    const { error } = await supabase
      ...
      .eq('id', photo.id)
    if (error) {
      console.error('[JobWalkPhotos] Caption update failed:', error)
      // Revert on error
      setPhotos((list) =>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 px-4 border border-dashed border-gray-200 rounded-lg">
            ...
            <p className="text-sm text-gray-400">
              No photos yet. Upload or take photos from your job walk.
            </p>
          </div>
        ) : (
