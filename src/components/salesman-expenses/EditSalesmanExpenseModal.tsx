...
interface EditSalesmanExpenseModalProps {
...
export default function EditSalesmanExpenseModal({
  expense,
  onClose,
  onUpdated,
}: EditSalesmanExpenseModalProps) {
  ...
  const existingReceiptUrl = expense.receipt_url
    ? supabase.storage.from('salesman-receipts').getPublicUrl(expense.receipt_url).data.publicUrl
    : null

  const [photoFile, setPhotoFile] = useState<File | null>(null)
  const [photoPreview, setPhotoPreview] = useState<string | null>(existingReceiptUrl)
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (removeExistingPhoto && expense.receipt_url) {
      ...
      if (photoFile) {
        ...
        const path = `${expense.user_id}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`
