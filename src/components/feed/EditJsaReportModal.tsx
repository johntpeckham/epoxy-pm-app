...
export default function EditJsaReportModal({
  ...
  const [projectName, setProjectName] = useState(initialContent.projectName ?? '')
  ...
  const [preparedBy, setPreparedBy] = useState(initialContent.preparedBy ?? '')
  const [siteSupervisor, setSiteSupervisor] = useState(initialContent.siteSupervisor ?? '')
  const [competentPerson, setCompetentPerson] = useState(initialContent.competentPerson ?? '')
  ...
  async function handleRefreshWeather() {
    ...
    const w = await fetchWeatherForAddress(address)
    if (w) setWeather(w)
