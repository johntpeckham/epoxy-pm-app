...
function InlinePhotoPost({ content, onImageClick }: { content: PhotoContent; onImageClick: (urls: string[], index: number) => void }) {
  ...
  return (
    <div className="mt-1 space-y-1.5 w-full">
      ...
      <div className="grid grid-cols-3 md:[grid-template-columns:repeat(5,minmax(120px,1fr))] gap-2 w-full">
        {urls.map((url, i) => (
    ...
    <div className="p-3.5 space-y-3 border-t border-amber-200 max-w-full overflow-hidden">
      ...
      {photoUrls.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-3 md:[grid-template-columns:repeat(5,minmax(120px,1fr))] gap-2">
            {photoUrls.map((url, i) => (
