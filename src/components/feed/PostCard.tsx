          ...
          <div>
            ...
            {photoUrls.length === 1 ? (
              ...
              <div className="grid grid-cols-6 gap-0.5">
                {photoUrls.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block">
                    <div className="relative w-[60px] h-[60px] rounded overflow-hidden bg-amber-50">
                      <Image
                        src={url}
          ...
          {post.post_type === 'daily_report' && (
            <div className="mt-1.5">
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-amber-200 bg-amber-50 hover:bg-amber-100 transition-colors text-left"
              >
                <ChevronRightIcon
                ...
                />
                <ClipboardListIcon className="w-3.5 h-3.5 text-amber-600 flex-shrink-0" />
                <span className="text-xs font-semibold text-amber-800">Daily Report</span>
                {(post.content as DailyReportContent).date && (
                  <span className="text-xs text-amber-600 tabular-nums">
                    {new Date((post.content as DailyReportContent).date + 'T12:00:00').toLocaleDateString('en-US', {
                      month: 'short',
                      day: 'numeric',
                      year: 'numeric',
                    })}
                ...
                {(post.content as DailyReportContent).project_foreman && (
                  <>
                    <span className="text-xs text-gray-500 mx-1">·</span>
                    <span className="text-xs text-gray-600 truncate">
                      Foreman: {(post.content as DailyReportContent).project_foreman}
                    </span>
                  </>
                )}
              </button>
