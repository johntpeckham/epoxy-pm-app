...
export interface MasterPriceCheckProfileOption {
  id: string
...
interface Props {
  productName: string
  supplierName: string
  profiles: MasterPriceCheckProfileOption[]
  onClose: () => void
  onSubmit: (assignedToId: string) => Promise<void> | void
}

export default function MasterPriceCheckRequestModal({
  productName,
  supplierName,
  profiles,
  onClose,
  ...
  async function handleSubmit(e?: React.FormEvent) {
    e?.preventDefault()
    if (!assignedTo) {
      setError('Please choose someone to assign the price check to.')
      return
    }
    setError(null)
    setSaving(true)
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-[#3a3a3a] flex-shrink-0">
            <div className="min-w-0 flex items-center gap-2">
              <DollarSignIcon className="w-5 h-5 text-amber-500 flex-shrink-0" />
