...
export default function MasterSettingsModal({
  ...
  async function handleEditSave() {
    ...
    setEditSaving(false)
    if (!error) {
      updateAndSync(unitTypes.map((u) => (u.id === editId ? { ...u, name, abbreviation: abbr } : u)))
      setEditId(null)
    }
  }
  ...
  async function confirmDelete() {
    ...
    setDeleting(true)
    const { error } = await supabase.from('unit_types').delete().eq('id', deleteTarget.id)
    setDeleting(false)
    if (!error) updateAndSync(unitTypes.filter((u) => u.id !== deleteTarget.id))
      ...
      >
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-[#3a3a3a] flex-shrink-0">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Material Management Settings</h2>
            <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-gray-600 dark:text-[#6b6b6b] dark:hover:text-white hover:bg-gray-100 dark:hover:bg-[#2e2e2e] rounded-md transition-colors">
          ...
          <div className="px-5 py-5 space-y-4 overflow-y-auto flex-1 min-h-0">
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-[#a0a0a0] uppercase tracking-wide mb-2">Unit Types</label>
              {unitTypes.length === 0 && !adding && (
                <p className="text-sm text-gray-400 dark:text-[#6b6b6b] mb-2">No unit types yet.</p>
              )}
          ...
          <div className="flex-none flex justify-end px-5 py-4 border-t border-gray-200 dark:border-[#3a3a3a]">
            <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-[#a0a0a0] bg-white dark:bg-[#2e2e2e] border border-gray-300 dark:border-[#3a3a3a] rounded-lg hover:bg-gray-50 dark:hover:bg-[#3a3a3a] transition">Done</button>
          </div>
        </div>
      </div>
