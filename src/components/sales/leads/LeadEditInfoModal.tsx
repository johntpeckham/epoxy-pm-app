...
export default function LeadEditInfoModal({
  ...
  async function handleSelectCustomer(c: Customer) {
    ...
    const { data: primary, error: contactErr } = await supabase
      ...
      .maybeSingle()
    if (contactErr) {
      console.error('[LeadEditInfoModal] Primary contact lookup failed:', {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(false)
    if (updateErr) {
      console.error('[Lead] Edit save failed:', updateErr)
    ...
    <Portal>
      ...
      >
        ...
        >
          ...
          >
            <h2 className="text-lg font-semibold text-gray-900">Edit Lead Info</h2>
            <button
              onClick={onClose}
              disabled={saving}
