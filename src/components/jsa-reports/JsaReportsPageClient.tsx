...
import { PlusIcon, ShieldIcon, SearchIcon, ChevronDownIcon, ChevronRightIcon, ArrowLeftIcon } from 'lucide-react'
...
function groupByProjectAndDate(reports: JsaReportRow[], sort: SortOption) {
  const projectMap = new Map<
    string,
    { projectName: string; dates: Map<string, JsaReportRow[]>; latestDate: string; oldestDate: string }
  >()
    ...
    <div>
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 pt-4 pb-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white truncate">JSA Reports</h1>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-500 font-medium">
            {searchQuery.trim() ? 'No reports match your search' : 'No JSA reports yet'}
          </p>
          <p className="text-gray-400 text-sm mt-1">
            {searchQuery.trim()
              ? 'Try a different search term.'
              : projects.length > 0
                ? 'Click "New JSA Report" to submit the first one.'
                : 'Create a project first, then submit JSA reports.'}
          </p>
        </div>
      ) : (
        <div>
